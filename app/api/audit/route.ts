...
export async function POST(request: NextRequest) {
  ...
  const installationId =
    typeof body?.installation_id === "string"
      ? body.installation_id.slice(0, 100)
      : "";
  const action =
    typeof body?.action === "string"
      ...
      : "app_event";
  if (!installationId) {
    return NextResponse.json({ error: "Aparelho inválido" }, { status: 400 });
  }
  ...
  const forwarded = request.headers
    ...
    ?.trim();
  const ipAddress =
    forwarded || request.headers.get("x-real-ip") || null;
  const location =
  ...
  if (deviceResult.error || eventResult.error) {
    return NextResponse.json(
      { error: deviceResult.error?.message || eventResult.error?.message },
      { status: 500 }
    );
  }
