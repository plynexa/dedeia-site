...
const categories=["Perfume","Colônia","Sabonete","Desodorante","Kit","Roupas","Acessórios"];
const empty={id:"",name:"",category:"Perfume",price:0,old_price:null,image_url:"",active:true};
...
export default function AdminPanel({initialProducts}:{initialProducts:Product[]}) {
  ...
  async function save(event:FormEvent<HTMLFormElement>){
    ...
    try{
      ...
      const payload={name:String(form.get("name")),category:String(form.get("category")),price:Number(form.get("price")),old_price:form.get("old_price")?Number(form.get("old_price")):null,image_url,active:true};
      const supabase=createClient();
      if(!supabase) throw new Error("Sem conexão");
      if(editing.id){
  ...
  return <main className="admin-page">
    ...
    {editing&&<div className="overlay"><section className="editor"><button className="close" onClick={()=>setEditing(null)}><X/></button><h2>{editing.id?"Editar produto":"Novo produto"}</h2><form onSubmit={save}>
      ...
      <label>Categoria<select name="category" defaultValue={editing.category}>{categories.map(c=><option key={c}>{c}</option>)}</select></label>
